...
namespace TourFacil\Core\Services\Integracao\NovaXS\Matria;
...
class MatriaService
{
    ...
    /** @var string  */
    protected $path = "integracao/matria/";

    /** @var string */
    const CRIANCA = "jovens";
     ...
     * @throws Exception
     */
    public function gerarVoucherMatria()
    {
        // Recupera os serviços disponível para a data do serviço
        $this->servicosDisponiveis = $this->matria->getProductsByDate([
        ...
        // Bloqueio de compra
        $this->buyToBillFor = $this->matria->buyToBillFor([
        ...
        $this->billFor = $this->matria->billFor([
        ...
        $this->getAccessList = $this->matria->getAccessList([
            'bill' => $this->buyToBillFor['id']
        ]);
    ...
    private function filterServicoMatria() {
        ...
        foreach ($this->servicosDisponiveis as $servico) {
            ...
            if(isset($servico['shortName'])) {
                ...
                foreach (self::TIPO_PESSOAS as $tipo_pessoa) {
                    ...
                    if($tipo_pessoa == self::SENIOR && $nome_servico = 'Ingresso jovens') {
                        $servicos[Str::slug('senior', "_")] = $servico['path'];
                    }
                }
            }
        }
    ...
    private function productsArray() {
        ...
        foreach ($this->reserva->quantidadeReserva as $quantidade_reserva) {
            ...
            if($quantidade_reserva->valor_net > 0) {
                ...
                /** Recupera os dados para o adulto */
                if(Str::contains($nome_variacao, "senior")) {
     ...
     * @return array
     */
    public function cancelarVoucher(MatriaReservaPedido $matriaReservaPedido)
    {
        // Solicita o cancelamento ao olivas
        $cancelado = $this->matria->cancelBill([
            'bill' => $matriaReservaPedido->bill_id
        ]);

        // atualiza o status do voucher para cancelado
        $matriaReservaPedido->update(['status' => IntegracaoEnum::VOUCHER_CANCELADO]);
